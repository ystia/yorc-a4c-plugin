...
#BINTRAY_API_KEY=AHAHAH

#### Bintray Variables
bintray_base_url="https://api.bintray.com/content"
bintray_api_user="stebenoist"
bintray_repo_distrib="ystia/yorc-a4c-plugin/distributions"
...
#### Artifacts Variables
artifacts_list="distribution/target/alien4cloud-yorc-plugin-distribution-*.zip alien4cloud-yorc-plugin/target/alien4cloud-yorc-plugin-*.zip"
distrib_artifact_dir="./distribution/target"
distrib_artifact_basename="alien4cloud-yorc-plugin-distribution-"

#### get_version ()
#### Allows to get the current distribution version
get_version () {
  find ${distrib_artifact_dir} -type f -name "${distrib_artifact_basename}*.zip" | awk -F${distrib_artifact_basename} '{print $2}' | sed "s|.zip||g" | sed "s|-SNAPSHOT||g"
}
